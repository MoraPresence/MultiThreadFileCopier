...
public:
    ...
    explicit producersFileCopier(std::string &dstDir, std::mutex *mutexConnection,
                                 std::atomic<bool> *atomicConnection)
            : _dstDir(dstDir),
              _overwrite(askOverwriteUserFlag),
              _mutexConnection(mutexConnection),
    ...
    producersFileCopier(std::string &dstDir, bool overwriteFlag,
                        std::mutex *mutexConnection, std::atomic<bool> *atomicConnection)
            : _dstDir(dstDir),
              _overwrite(overwriteFlag),
              _mutexConnection(mutexConnection),
    ...
    int copyFilesToAnotherDir(struct directoryInfo *, std::string);
